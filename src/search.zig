...
const Item = struct { idx: usize, str: []const u8 };
...
const Matcher = struct {
    ...
    big_items: ArrayList(Item),

    fn init(allocator: std.mem.Allocator) Matcher {
        return Matcher{ .bucket_8 = Bucket(8){ .items = ArrayList(BucketItem(8)).init(allocator) }, .bucket_16 = Bucket(16){ .items = ArrayList(BucketItem(16)).init(allocator) }, .bucket_32 = Bucket(32){ .items = ArrayList(BucketItem(32)).init(allocator) }, .bucket_64 = Bucket(64){ .items = ArrayList(BucketItem(64)).init(allocator) }, .bucket_128 = Bucket(128){ .items = ArrayList(BucketItem(128)).init(allocator) }, .bucket_256 = Bucket(256){ .items = ArrayList(BucketItem(256)).init(allocator) }, .bucket_512 = Bucket(512){ .items = ArrayList(BucketItem(512)).init(allocator) }, .big_items = ArrayList(Item).init(allocator) };
    }
...
fn find_needle_in_matcher(allocator: std.mem.Allocator, needle: []const u8, matcher: Matcher, scores: *ArrayList(Match)) void {
    ...
    for (matcher.big_items.items) |item| {
        scores.append(Match{
            .score = regular_search.smith_waterman(allocator, needle, item.str),
            .idx = item.idx,
        }) catch unreachable;
    }
}
    ...
    for (0..haystacks.len) |idx| {
        ...
        switch (haystack.len) {
            ...
            257...512 => {
                ...
                matcher.big_items.append(Item{ .idx = idx, .str = haystack }) catch unreachable;
            },
        }
    }
